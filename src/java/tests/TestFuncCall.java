...
public class TestFuncCall extends AbstractParameterizedTest{
	/**
	 * Directory where test files live
	 */
	private static final String TEST_DIR = "testfiles/funccalls";

	/**
	 * Construct a new JastAdd test
	 * @param testFile filename of test input file
	 */
	public TestFuncCall(String testFile) {
	...
	public void runTest() throws Exception {
		...
		try{
			...
			for(int i = 0; i < program.getNumFunc(); i++){
				...
				System.out.println(func.getFuncName().getID() + " encloses:");
				for(Func f: func.functionCalls()){
